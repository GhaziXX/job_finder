  ...
  Future updatePosition() async {
    await databaseReference.get().then((QuerySnapshot snapshot) {
      ...
      for (var i = 0; i < l; i++) {
        if (d[i].data()['uid'] == uid) {
          try {
            databaseReference.doc(d[i].id).update(
                {'position': GeoPoint(double.parse(lat), double.parse(lng))});
          } catch (e) {
            print(e.toString());
          }
          break;
  ...
  void getLoc() async {
    await databaseReference.get().then((QuerySnapshot snapshot) {
      ...
      for (int i = 0; i < d.length; i++) {
        ...
        if (da['uid'] == uid) {
          if (d[i].data().containsKey("position")) {
            GeoPoint geoPoint = d[i].get("position");
            setState(() {
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return InkWell(
                              ...
                              child: RecentJobCard(
                                company: recent,
                                isBook:
                                    books.contains(recent.id) ? true : false,
                              ),
                            );
                          },
                        );
